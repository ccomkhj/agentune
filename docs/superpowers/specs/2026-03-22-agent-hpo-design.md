...
|---|---|---|
| **CLI** (`agent-hpo`) | Thin user-facing commands over core | Core service layer |
...
1. A study spec exists (search space, budget). For round 1, `agent-hpo init` creates it automatically from campaign defaults (`initial_search_space`, `trials_per_round`). For subsequent rounds, the agent proposes it via `submit_action_proposal`.
...
`agent-hpo run` drives one round and returns at `AWAITING_AGENT`, `COMPLETED`, or `FAILED`. Scheduler is an internal module — no long-running daemon process in v0.
...
**Transitions:**
- `CREATED → RUNNING`: Triggered by `agent-hpo run` when the first round begins
- `RUNNING → PAUSE_REQUESTED`: Triggered by `agent-hpo pause`. Sets a persisted flag on the campaign row (`pause_requested = true`). The scheduler checks this flag after each round completes and transitions to `PAUSED` instead of continuing.
- `PAUSE_REQUESTED → PAUSED`: Applied by scheduler when the active round reaches `AWAITING_AGENT` or `CLOSED`. The flag is cleared.
- `PAUSED → RUNNING`: Triggered by `agent-hpo resume`
...
- `RUNNING → STOPPED`: Triggered by `agent-hpo stop` (manual operator intervention). Terminal state — cannot be resumed. Differs from `COMPLETED` in intent: `STOPPED` means "aborted by operator," `COMPLETED` means "goal met or stop condition fired."
...
- `PROPOSED`: Round created, not yet executing. Round 1 is system-generated by `agent-hpo init` from campaign defaults (no agent proposal needed, `references` is empty). Subsequent rounds are created from accepted agent proposals.
...
|---|---|
| `agent-hpo init` | Create a campaign (metric, direction, backend, improvement criteria, sampler, stop conditions) |
| `agent-hpo run` | Execute current/next study round — returns at AWAITING_AGENT, COMPLETED, or FAILED |
| `agent-hpo status` | Print campaign state + latest round summary |
| `agent-hpo history` | Print all rounds and decisions |
| `agent-hpo pause` | Transition campaign to PAUSED |
| `agent-hpo resume` | Transition campaign from PAUSED to RUNNING |
| `agent-hpo stop` | Manually stop a campaign |
| `agent-hpo export` | Export best params, model artifact path |
| `agent-hpo baseline` | Run a single plain Optuna study with same total budget — the comparison target |
...
**Comparison:**
1. Run `agent-hpo baseline` — plain Optuna, same total budget
